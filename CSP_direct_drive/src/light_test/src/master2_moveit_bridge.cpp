...
constexpr std::int64_t kCycleTimeNs = 10000000;
...
public:
  ...
  Master2MoveItBridge() : Node("master2_moveit_bridge") {
    ...
    joint_state_pub_ = create_publisher<sensor_msgs::msg::JointState>(
        ("/" + topic_prefix_ + "/joint_states"), 10);
    raw_count_pub_ = create_publisher<std_msgs::msg::Int64MultiArray>(
        ("/" + topic_prefix_ + "/encoder_counts"), 10);
    estop_state_pub_ = create_publisher<std_msgs::msg::Bool>(
        ("/" + topic_prefix_ + "/emergency_stop_state"), rclcpp::QoS(1).transient_local());
    enabled_state_pub_ = create_publisher<std_msgs::msg::Bool>(
        ("/" + topic_prefix_ + "/motors_enabled"), rclcpp::QoS(1).transient_local());

    estop_sub_ = create_subscription<std_msgs::msg::Bool>(
        ("/" + topic_prefix_ + "/emergency_stop"), rclcpp::QoS(10),
    ...
    trajectory_sub_ = create_subscription<trajectory_msgs::msg::JointTrajectory>(
        ("/" + topic_prefix_ + "/joint_trajectory"), 10,
    ...
    enable_service_ = create_service<std_srvs::srv::SetBool>(
        ...
        [this](const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
          ...
          if (estop_latched_.load()) {
            ...
            for (std::size_t i = 0; i < kJointCount; ++i) {
              const auto logical_actual = logical_actual_count(i);
              targets_[i].store(logical_actual);
              commanded_[i].store(logical_actual);
            }
    ...
    reset_estop_service_ = create_service<std_srvs::srv::Trigger>(
        ("/" + topic_prefix_ + "/reset_emergency_stop"),
...
private:
  ...
  void update_all_axes() {
    ...
    for (std::size_t i = 0; i < count; ++i) {
      ...
      if (error != 0 && !controller_enabled_.load(std::memory_order_relaxed) &&
          (status & 0x0008)) {
        *axis.target_position = raw_target_count(i, logical_actual_count(i));
      ...
      if (error != 0) {
        if (!estop_latched_.load(std::memory_order_relaxed)) {
          RCLCPP_ERROR(
              get_logger(),
              "%s drive fault: status=0x%04x error=0x%04x raw=%d logical=%ld target=%d",
              joint_names_[i].c_str(), static_cast<unsigned>(status),
              static_cast<unsigned>(error), actual, logical_actual_count(i),
              targets_[i].load(std::memory_order_relaxed));
        }
        trip_estop_rt(100 + static_cast<int>(i));
      }
      if (limit_enabled_[i] &&
          !limit_count_offsets_ready_[i].load(std::memory_order_relaxed)) {
        const auto offset = select_limit_offset(i, actual);
        if (!offset.has_value()) {
          trip_estop_rt(250 + static_cast<int>(i));
        } else {
          limit_count_offsets_[i].store(*offset, std::memory_order_relaxed);
          limit_count_offsets_ready_[i].store(true, std::memory_order_relaxed);
          if (*offset != 0) {
            RCLCPP_WARN(
                get_logger(),
                "%s startup interval offset selected: %+ld counts",
      ...
      if (estop_latched_.load(std::memory_order_relaxed)) {
        *axis.target_position = raw_target_count(i, logical_actual);
        *axis.control_word = enabled ? 0x000b : 0x0006;
        continue;
      }
      if (!controller_enabled_.load(std::memory_order_relaxed)) {
        *axis.target_position = raw_target_count(i, logical_actual);
        commanded_[i].store(logical_actual, std::memory_order_relaxed);
      ...
      if (status & 0x0008) {
        ...
        *axis.target_position = raw_target_count(i, next);
        if (std::llabs(logical_actual - static_cast<std::int64_t>(next)) >
            following_error_counts_) {
  ...
  std::int64_t count_period(std::size_t i) const {
    return static_cast<std::int64_t>(
        std::llround(counts_per_rev_[i] * gear_ratios_[i]));
  }

  std::optional<std::int64_t> select_limit_offset(
      std::size_t i, std::int32_t actual) const {
    if (!limit_enabled_[i]) {
      return 0;
    }
    const auto period = count_period(i);
    if (period <= 0) {
      return std::nullopt;
    }
    const auto raw = static_cast<std::int64_t>(actual);
    if (raw >= raw_min_[i] && raw <= raw_max_[i]) {
      return 0;
    }
    const double min_shift_d =
        (static_cast<double>(raw_min_[i]) - static_cast<double>(raw)) /
        static_cast<double>(period);
    const double max_shift_d =
        (static_cast<double>(raw_max_[i]) - static_cast<double>(raw)) /
        static_cast<double>(period);
    const auto min_shift = static_cast<std::int64_t>(std::ceil(min_shift_d));
    const auto max_shift = static_cast<std::int64_t>(std::floor(max_shift_d));
    if (min_shift > max_shift) {
      return std::nullopt;
    }
    const auto shift = (0 >= min_shift && 0 <= max_shift)
                           ? std::int64_t{0}
                           : (min_shift > 0 ? min_shift : max_shift);
    const auto shifted = raw + shift * period;
    if (shifted < raw_min_[i] || shifted > raw_max_[i]) {
      return std::nullopt;
    }
    return shift * period;
  }

  std::int64_t logical_actual_count(std::size_t i) const {
    return static_cast<std::int64_t>(actual_positions_[i].load(
               std::memory_order_relaxed)) +
           limit_count_offsets_[i].load(std::memory_order_relaxed);
  }

  std::int32_t raw_target_count(std::size_t i, std::int64_t logical_count) const {
    const auto raw = logical_count -
                     limit_count_offsets_[i].load(std::memory_order_relaxed);
  ...
  bool positions_safe(std::string &error) const {
    for (std::size_t i = 0; i < kJointCount; ++i) {
      ...
      const auto actual = logical_actual_count(i);
  ...
  bool validate_trajectory(
    ...
    for (std::size_t i = 0; i < kJointCount; ++i) {
      previous_counts[i] = static_cast<std::int32_t>(logical_actual_count(i));
    }
  ...
  void set_targets_from_radians(
    ...
    for (std::size_t i = 0; i < kJointCount; ++i) {
      const auto reference = target_valid_.load()
                                 ? targets_[i].load()
                                 : static_cast<std::int32_t>(logical_actual_count(i));
  ...
  std::array<double, kJointCount> actual_radians() const {
    ...
    for (std::size_t i = 0; i < kJointCount; ++i) {
      values[i] = count_to_rad(i, logical_actual_count(i));
    }
    return values;
  }
  ...
  void hold_current_position() {
    for (std::size_t i = 0; i < kJointCount; ++i) {
      targets_[i].store(logical_actual_count(i));
    }
    target_valid_.store(true);
  }
  ...
  bool execute_trajectory(
    ...
    while (rclcpp::ok() && std::chrono::steady_clock::now() < deadline) {
      bool reached = true;
      for (std::size_t i = 0; i < kJointCount; ++i) {
        if (std::llabs(logical_actual_count(i) - targets_[i].load()) >
            goal_tolerance_counts_) {
          reached = false;
        }
      }
  ...
  void publish_state() {
    ...
    for (std::size_t i = 0; i < kJointCount; ++i) {
      ...
      const auto logical_position = logical_actual_count(i);
      joint_state.position[i] = count_to_rad(i, logical_position);
  ...
  std::array<std::atomic<std::int64_t>, kJointCount> limit_count_offsets_{};
  std::array<std::atomic<bool>, kJointCount> limit_count_offsets_ready_{};
